...
- `_core/web_browsing` currently uses the transient sections seam in two narrow ways: one hook injects a brief `currently open web browsers` status block when browser windows are open, listing the public numeric browser ids plus their latest url and title, and a second hook injects `last interacted web browser` only when a remembered interacted browser is still open at prompt-build time, loading fresh simplified page content then rather than caching it at interaction time; when those conditions are not met the owner-module hooks should add nothing
