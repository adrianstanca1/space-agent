...
- `view.html`: routed spaces canvas shell, the widget-grid mount point, the onboarding stylesheet import, the route-owned current-space header teleport markup, and the live `space:open` skill-context tag while a current space is open
- `store.js`: spaces store, route-driven loading, runtime namespace registration, current-space replay, current-space topbar actions including confirmed bulk widget clearing, widget-card lifecycle, empty-space multi-step onboarding choreography, current-space metadata autosave, renderer cleanup, and direct-manipulation layout interactions
...
- the empty-space onboarding stack should sit above the viewport midpoint with explicit top and bottom breathing room so the reserved example-button area stays higher and the CTA copy can finish below it without slipping into the onscreen-agent overlay zone
...
- the routed spaces page should keep its own chrome out of the canvas body and instead define its current-space control cluster directly inside `view.html`, then teleport that route-owned markup into the menu shell's existing `[id="_core/onscreen_menu/bar_start"]` target, with a Back action, the title toggle, a Rearrange action, and an icon-only clear-all-widgets action whose confirmation stays in the spaces store instead of the template
- the teleported current-space header controls should rely on the shared bar chrome instead of adding module-local borders or filled button backgrounds unless a specific interaction state truly requires it
...
- when the visible widget span is shorter than the viewport, give both camera axes real sparse-layout wiggle room instead of snapping back to a locked midpoint; horizontal sparse content should still start visually centered but remain pannable left or right afterward, and vertical sparse mode should also take over whenever the normal content-fit clamp would leave only tiny travel on medium-height screens, so a lone widget does not feel vertically locked just because the natural fit range technically still exists; vertical sparse content should start slightly scrolled downward so first widgets sit higher on the screen, and returning to the empty canvas should reset the camera before the next first-widget render so stale offsets do not leak into the next sparse layout
...
- the routed header control cluster should expose a chevron-style back icon button, the current space title toggle, the rearrange icon button, and an icon-only trash-style clear-all-widgets button through the menu shell's existing `[id="_core/onscreen_menu/bar_start"]` container; the title toggle should anchor the current-space metadata popover from the toggle button itself instead of opening a separate side drawer, the clear-all action should confirm before calling the existing `removeAllWidgets()` runtime path, and the rearrange action should recenter the camera at `0,0`, preserve minimized widgets, and rewrite widget positions into a centered packed layout that scans cells left to right and top to bottom, skips occupied cells immediately, and at each free cell places the largest remaining widget that physically fits within the viewport-width column threshold before moving onward without skipping that row for shape heuristics
...
- `_core/spaces` injects the always-available New Space topbar action through `_core/dashboard/topbar_primary`, plus the existing-space list, bottom footer metadata row, duplicate action, bottom-right trash-can delete action, and New Space card through `_core/dashboard/content_middle`; the topbar action should stay first in the dashboard's teleported control cluster and should use the same create-space flow as the launcher card, the spaces section should stay above the dashboard pages section, and regular space cards should feel obviously clickable with pointer and press feedback, while the create card should stand apart with a centered larger icon, a bottom-centered label, and a restrained background glow rather than a loud gradient wash
